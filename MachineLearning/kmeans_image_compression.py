...
class ImageCompressor:
    ...
    def compress_image(self, img, num_colors):
        ...
        for i in range (len(x)):
            storeColor = labels[i]
            changeColor = centroids[storeColor]
            x[i] = changeColor
        return self.convert_to_2D(x)

if __name__ == '__main__':
    # TODO
    imageCompressed = ImageCompressor()
    originalImage = imageCompressed.load_image("Bridge.jpeg")
    newImage = imageCompressed.load_image("Bridge.jpeg")
    compressImage = imageCompressed.compress_image(newImage, 10)
    imageCompressed.plot_image_comparisons(originalImage, compressImage)
